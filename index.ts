...
async function main() {
  ...
  appUserConfig.preferredThemeMode === "dark" &&
  ...
  const initUnsplash = () => {
    ...
    const fetchDataFromUnsplash = async (searchTerm: string) => {
      ...
      const params = `?query=${encodeURIComponent(
        searchTerm
      )}&per_page=30&page=${currentPage}&client_id=${apiKey}`;
  ...
  logseq.Editor.registerSlashCommand("Unsplash", async () => {
